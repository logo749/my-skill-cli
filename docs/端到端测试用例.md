...
async function test() {
  ...
  console.log('客户端状态:', status.running ? '运行中' : '已停止');
}
...
**前置条件**：服务已启动，配置了 LOGIN_STATUS_URL

**配置示例**：

```bash
# .env 文件
LOGIN_STATUS_URL=https://api.example.com/check-login
...
# 3. 检查健康接口返回登录状态
curl http://localhost:3000/health
# 预期: {"status":"ok","port":3000,"isLoggedIn":true}
...
async function test() {
  const client = createClient({
    loginStatusUrl: 'https://api.example.com/check-login',
    loginStatusInterval: 5000,
    onLog: (msg) => console.log('[LOG]', msg)
  });

  await client.start();
  console.log('服务已启动，登录状态检测已启动');

  // 检查初始状态
  const status = client.getStatus();
  console.log('初始状态:', status.isLoggedIn);
  ...
  console.log('当前状态:', status2.isLoggedIn);
}
